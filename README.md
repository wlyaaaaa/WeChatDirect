...
删除约 1300 行没有任何命令、测试或文档调用的读取器代码（群锚点投影、群本人回执和消息目录准备）。归档事务改用文件大小、修改时间和文件系统变更时间判断并发改动，来源未变时不再整树读取和哈希；复制、校验、改名和清理遇到文件被占用统一返回 `archive_output_in_use`。测试统一屏蔽本机设置和 `WECHAT_DIRECT_*` 环境变量；朋友圈身份去后缀规则只保留一处实现。
