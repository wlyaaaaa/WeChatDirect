        ...
        *,
        ...
        for source, connection in self._message_connections(table):
            ...
            if server_index is None:
                raise DirectSchemaError("quote_server_index_unavailable")
            rows = connection.execute(
                "SELECT local_id, local_type, server_id, real_sender_id, create_time, "
                "message_content, source, packed_info_data, compress_content, sort_seq, "
                f"{status_expr} AS status, {origin_expr} AS origin_source "
                f"FROM {quoted_table} INDEXED BY {_quote_identifier(server_index)} "
        ...
        if len(canonical) != 1:
            raise DirectSchemaError("quote target identity is conflicting")
        return projected[0]
    ...
    def fetch_messages(
        ...
        *,
        ...
        for source, connection in message_connections:
            ...
            fallback_sort_indexes: set[str] = set()
            if since_s is not None and since_sort_seq is not None:
                ...
                if time_index is None:
                    ...
                    fallback_sort_indexes.add(sort_index)
                else:
                    predicates.append(
                        (
                            "create_time >= ? AND create_time <= ?",
            ...
            for where, params, index_name in predicates:
                ...
                if limit is not None:
                    order_and_limit = (
                        ...
                        if index_name in fallback_sort_indexes
                        else " ORDER BY (create_time IS NULL), create_time DESC, "
                        "sort_seq DESC, server_id DESC, local_id DESC LIMIT ?"
                    )
                ...
                cursor = connection.execute(
                    ...
                    "message_content, source, packed_info_data, compress_content, sort_seq, "
                    f"{status_expr} AS status, {origin_expr} AS origin_source "
                    f"FROM {quoted_table}{indexed_by} WHERE {where}{order_and_limit}",
                    query_params,
                )
        ...
        if limit is not None:
            ...
            unique_rows: list[tuple[Path, sqlite3.Row]] = []
        ...
        for message_source, row in rows:
            identity = self._message_row_identity(row) or (
                "local",
                str(row["local_id"]),
            )
            messages.setdefault(
                ...
                self._message_from_row(
                    ...
                    message_source=message_source,
                    message_table=table,
                    connection=connection_by_source[message_source],
                    sender_index=self._sender_index_for_message_source(
                        message_source
                    ),
                    exact_media_lookup=exact_media_lookup,
                ),
